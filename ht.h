...
{
    double alpha = static_cast<double>(filled_)/table_.size();
    if (alpha >= resizeAlpha_){
        resize();
    }
    if (find(p.first) && !internalFind(p.first)->deleted){
        find(p.first)->second = p.second;
        return;
    }
    HASH_INDEX_T h = probe(p.first);
    if (h == Prober::npos){
        throw std::logic_error("cannot be inserted");
    }
    HashItem* nh = new HashItem(p);
    table_[h] = nh;
    filled_ += 1;
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    std::vector<HashItem*> temp = table_; // copy over old table
    int oldSize = CAPACITIES[mIndex_];
    int newSize = CAPACITIES[mIndex_+1];
    mIndex_++;
    filled_ = 0;
    deleted_ = 0;
    table_.clear();
